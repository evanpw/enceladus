...
void StackAlloc::run()
{
    ...
    if (neededRoom != 0)
    {
        ...
        VirtualRegister* rsp = _function->makePrecoloredReg(_context->rsp, NotReference);
        MachineInst* allocInst = new MachineInst(Opcode::ADD, {rsp}, {rsp, _context->makeImmediate(-neededRoom)});
